...
In the graph above, it's not unimodal but in fact is bimodal, seeing that there is only 2 peaks. Both peaks are not the same   size. The sample mean is `r mean(my1000)` and the standard devination is `r sd(my1000)`
...
Describe the resulting plot -  is it unimodal?  How many peaks does it have?  Are all the peaks the same size? 
 ...
 The resulting plot is a trimodal, where there are three peaks. All three spikes are not the same size. The sample mean is `r mean(x.avg)` and the standard devination is `r sd(x.avg)`
...
The sample mean is `r mean(x.avg)` and the standard devination is `r sd(x.avg)`
...
When viewing all the graphs and random sample size in each graph, there is a pattern that slowly agrees with the theory of CLT.
